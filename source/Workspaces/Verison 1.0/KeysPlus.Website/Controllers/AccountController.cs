...
namespace KeysPlus.Website.Controllers
{
    ...
    public class AccountController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.IsSuccess)
                {
                    ...
                    switch (userRole)
                    {
                        ...
                        case 6:
                            return RedirectToAction("GetMarketJobs", "Home", new { Area = "Jobs" });
                        default:
                            return RedirectToAction("Index", "Home", new { Area = "Rental" });
                    }
                }
                else
                {
        ...
        public ActionResult Activate(string token, int userId)
        {
            ...
            {
                if (activateUser.IsActive == true && activateUser.EmailConfirmed == true)
                {
        ...
        public ActionResult AccountOverview()
        {
            ...
            model.Profile.IsServiceSupplier = AccountService.IsServSupplier(currentUser);
        ...
        [HttpGet]
        public JsonResult FetchAccountOverview()
        {
            var username = User.Identity.Name;
            var currentUser = AccountService.GetLoginByEmail(username);
            Login getloginuser = db.Login.FirstOrDefault(u => u.UserName.Equals(username));
            Person personData = db.Person.FirstOrDefault(us => us.Id.Equals(getloginuser.Id));
            ServiceProvider serviceSuppData = db.ServiceProvider.FirstOrDefault(s => s.Id.Equals(personData.Id));
            var companyId = serviceSuppData.CompanyId;
            Company companyData = db.Company.FirstOrDefault(c => c.Id == companyId);
            Address getPhyaddress = db.Address.FirstOrDefault(x => x.AddressId.Equals(personData.PhysicalAddressId));
            Address getBilladdress = db.Address.FirstOrDefault(x => x.AddressId.Equals(personData.BillingAddressId));
            Address getCompPhyAddress = db.Address.FirstOrDefault(w => w.AddressId == companyData.PhysicalAddressId);
            Address getCompBillAddress = db.Address.FirstOrDefault(w => w.AddressId == companyData.BillingAddressId);
            AccountOverviewModel model = new AccountOverviewModel();
            try
            {
                ...
                model.PlaceOfBirth = personData.PlaceOfBirth;
                model.Language = personData.Language;
                model.Occupation = personData.Occupation;
                model.LinkedinUrl = personData.LinkedinUrl;
                model.PhoneNumber = personData.Login.PhoneNumber;
                model.IsServiceSupplier = AccountService.IsServSupplier(currentUser);                
                //model.ProfilePhoto = personData.ProfilePhoto;
                model.ProfilePhotoModel = new MediaModel { Data = Url.Content(MediaService.GetContentPath(personData.ProfilePhoto)), Status = "load" };

                model.MediaFiles = personData.PersonMedia.Select(y => new MediaModel
                {
                    Data = Url.Content(MediaService.GetContentPath(y.NewFilename)),
                    Id = y.Id,
                    MediaType = MediaService.GetMediaType(y.OldFilename),
                ...
                model.PhysicalAddress.Number = getPhyaddress.Number;
                model.PhysicalAddress.Street = getPhyaddress.Street;
                model.PhysicalAddress.Suburb = getPhyaddress.Suburb;
                model.PhysicalAddress.City = getPhyaddress.City;
                model.PhysicalAddress.PostCode = getPhyaddress.PostCode;
                model.PhysicalAddress.Longitude = getPhyaddress.Lng;
                model.PhysicalAddress.Latitude = getPhyaddress.Lat;

                model.BillingAddress.Number = getBilladdress.Number;
                model.BillingAddress.Street = getBilladdress.Street;
                model.BillingAddress.Suburb = getBilladdress.Suburb;
                model.BillingAddress.City = getBilladdress.City;
                model.BillingAddress.PostCode = getBilladdress.PostCode;
                model.BillingAddress.Longitude = getPhyaddress.Lng;
                model.BillingAddress.Latitude = getPhyaddress.Lat;
        ...
        [HttpPost]
        public ActionResult Edit(AccountOverviewModel model)
        {
            int roleId=0;
            ...
            else
            {
                ...
                //model.ProfilePhoto = "~/images" + file.FileName;
                //person.ProfilePhoto = model.ProfilePhoto;
                //...................................................................
                //var saveResultProfile = MediaService.SaveMediaFilesWithFilter(files, 5, "NewPhoto");//MediaService.SaveMediaFiles(model.NewPhoto);
                var saveResultProfile = MediaService.SaveMediaFilesWithFilterAndRotate(files, 5, "NewPhoto");

                if (!(model.ProfilePhotoModel == null))
                {
                    if (model.ProfilePhotoModel.Status == "removed" )
                    {
                ...
                //var saveResult = MediaService.SaveMediaFilesWithFilter(files, 5, "MediaFiles");
                var saveResult = MediaService.SaveMediaFilesWithFilterAndRotate(files, 5, "MediaFiles");
                if (saveResult != null)
                {
                ...
                if (model.FilesRemoved != null)
                {
                    foreach (var item in model.FilesRemoved)
                    {
                        ...
                        if (media != null)
                        {
                            ...
                            System.IO.File.Delete(Path.Combine(Server.MapPath("~/documents"), media.NewFilename));
                        }
                    }
                }
        ...
        {
            var serviceSupplier = db.ServiceProvider.Where(s => s.Id == model.Id).FirstOrDefault();
            var company = db.Company.Where(p => p.Id == serviceSupplier.CompanyId).FirstOrDefault();
            var status = true;
            var message = "";
            //var file = model.Image;
            var files = Request.Files;
            ...
            else
            {
                ...
                var saveResultProfile = MediaService.SaveMediaFilesWithFilterAndRotate(files, 5, "NewCompPhoto");

                if (!(model.ProfilePhoto == null))
                {
                    if (model.ProfilePhoto.Status == "removed")
                    {
                ...
                //    var newOb = saveResult.NewObject as List<MediaModel>;
                //    var mediaresult = AccountService.SaveCompanyPhoto(newOb, model.Id);
                //}
