...
namespace KeysPlus.Website.Areas.Jobs.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult GetQuoteMarketJob(int jobId, string searchString, string sortOrder, int page = 1, bool isOwner = false)
        {
            var currentJob = db.TenantJobRequest.Where(x => x.Id == jobId).FirstOrDefault();
            // var jobQuotesForJob;
            if (currentJob != null)
            {
                var jobQuotesForJob = db.JobQuote.Where(x => x.TenantJobRequest == currentJob).ToList().ToPagedList(page, 10);
                return View(jobQuotesForJob);
            }
            //  elsed
            return null;
        }
        ...
        public JsonResult SaveJob(JobModel jobViewModel)
        {
            ...
            {
                var dupeJob = db.Job.FirstOrDefault(x => x.JobDescription == jobViewModel.JobDescription && x.PropertyId == jobViewModel.PropertyId && x.JobStatusId != 4 && x.JobStatusId != 5);
                if (dupeJob != null)
                {
                    //duplicate means there is a duplicate record exsiting in database
                    //there should be duplicate pop up notice displaying
        ...
        public ActionResult GetJobQuotes(QuotesSearchViewModel model)
        {
            ...
            sortOrders.Add(new SortOrderModel { SortOrder = "Higest Amount", ActionName = "GetJobQuotes", RouteValues = rvr.AddRouteValue("SortOrder", "Higest Amount") });
