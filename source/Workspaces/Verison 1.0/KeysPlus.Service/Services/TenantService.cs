    ...
    public class TenantService
    {
        ...
        public static IEnumerable<TenantRentalViewModel> GetMyRentals(int loginId)
        {
            using (var db = new KeysEntities())
            {
                ...
                var rps = allRps.Where(tp => tp.TenantId == loginId && (tp.IsActive ?? true)).ToList();
        ...
        public static Tenant GetTenantByLoginId(int loginId)
        {
            using (var db = new KeysEntities())
            {
                var login = db.Login.FirstOrDefault(x => x.Id == loginId);
                return login == null ? null : db.Tenant.FirstOrDefault(x => x.Id == loginId);
            }
        }
